...
static BOOL PumpRealPageTurnBrokerEntry(
    ...
    if ([entry.deadline timeIntervalSinceNow] <= 0) {
        ...
        NSString *domTextPreview = domText.length > 180
            ...
            : domText;
        NSArray *resources = [transportSnapshot[@"resources"] isKindOfClass:[NSArray class]]
            ? transportSnapshot[@"resources"]
            : @[];
        NSArray *resourceTail = resources.count > 12
            ? [resources subarrayWithRange:NSMakeRange(resources.count - 12, 12)]
            : resources;
        NSString *detail = [NSString stringWithFormat:
