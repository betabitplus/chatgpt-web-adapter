...
def test_temporary_probe_is_layered_above_reconciled_worker() -> None:
    ...
    history_worker = (
        root / "service_worker_temporary_chat_history_probe.js"
    ).read_text(encoding="utf-8")
    assert (
        'importScripts("service_worker_temporary_chat_turn_probe.js")' in history_worker
    )
    ...
    semantic_worker = (
        root / "service_worker_temporary_chat_semantic_notice.js"
    ).read_text(encoding="utf-8")
    assert (
    ...
    assert (
        'importScripts("service_worker_temporary_chat_state_semantics.js")' in ax_worker
    )
    state_worker = (
        root / "service_worker_temporary_chat_state_semantics.js"
    ).read_text(encoding="utf-8")
    assert 'importScripts("service_worker_temporary_chat.js")' in state_worker
    base_worker = (root / "service_worker_temporary_chat.js").read_text(
        encoding="utf-8"
    )
    assert (
        'importScripts("service_worker_runtime_tab_reconciliation.js")' in base_worker
    )
