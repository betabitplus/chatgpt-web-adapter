...
- Final implementation commits are CWA `76439fe6691a42220083b9b8aaef1aad79671525` (`experiment/wk-passive-original-stream`) and gptty `40748c03935bbda47baedb3910368d3c7cf33a7b` (`feature/wkwebview-authority-e2e`). CWA was pushed to both user-owned `origin` and private `backup`; gptty was pushed to the user-owned `origin`. The installed `/Users/stas/.local/bin/gptty` was then replaced with a clean wheel built from exact gptty commit `40748c0` without dependency reinstall. Installed `commands/chat.py` and `sdk_client.py` SHA-256 hashes match the commit byte-for-byte, while CWA remains editable from the exact WK checkout carrying `76439fe`.
...
- Deterministic regression coverage proves that a stream item received after an intermediate raw `done` is still delivered, and product-runtime coverage proves that the stream stop condition switches only after the canonical assistant `end_turn`. Targeted CWA tests are 74/74, full CWA is 2274/2274, clean downstream gptty is 292/292 after the superseded release-staging tests were archived separately, and targeted Ruff is clean.
- Installed-artifact live verification then used an isolated tool-heavy conversation `6aa539b5-e484-83ed-81e8-71bd5d1cd8e8`. The harness took the early `browser_native_write_identity_resolved` conversation identity, attached the installed `/Users/stas/.local/bin/gptty` through `/resume` while canonical status was still `tool_calling` / `tool_running`, entered `Following active response via live stream…` on topic `conversation-turn-2b363f38-d611-4425-8c03-667d22413d5a`, remained alive through the active turn, and rendered the final `WK_SEGMENT_FINAL_OK_20260912` marker from bytes received only after the live-stream banner. The sender independently completed with the same marker. A parallel raw follower observed `raw_done_count=0` in this particular turn, so this run is recorded as installed end-to-end live-follow evidence, not as an additional live `done -> next frame` proof; the latter remains covered by the deterministic regression above.

### 2026-09-12 — resumed observer completeness repair

- User-visible comparison on `6aa4861b-475c-83eb-b7da-4406ce8c8b3b` showed that the WK observer stayed alive but omitted a substantial fraction of the reasoning updates visible in ChatGPT web. Canonical forensic comparison of the latest turn found 38 current-branch nodes: 13 tool calls, 13 tool results, 8 `content_type="thoughts"` reasoning summaries, 3 thinking preambles, and 1 user message. Before the repair, observer classification covered 29/38 nodes: every tool call/result and preamble, but none of the eight `thoughts` summaries.
- The missing public surface is stored as `content.thoughts[].summary`; `extract_message_text()` previously ignored the `thoughts` container entirely. The repair special-cases `content_type="thoughts"` and extracts only `thoughts[].summary`, deliberately ignoring `parts`, `thoughts[].content`, `chunks`, and other non-summary fields. Snapshot classification and `CanonicalTopicStreamNormalizer` now emit those summaries as `canonical_intermediate_message(message_kind="reasoning")`, including title-less summaries. Celsius patch handling also reconstructs `/message/content/thoughts/<n>/summary` fragments before completion.
- A second completeness defect suppressed every revision-sensitive canonical `current_node` even after that message itself reported a completion signal. The guard now suppresses only unfinished current reasoning/preamble messages; a `finished_successfully` current reasoning summary is safe to emit. On the exact reported conversation, the same forensic comparison then reached 37/38 coverage: all 37 non-user nodes were represented by the observer and only the user message remained intentionally outside observer events.
- Regression gates cover public-summary-only extraction, suppression of non-summary thought payload fields, canonical `get_messages()` history, title-less snapshot reasoning, live full-message reasoning, live summary patch reconstruction, unfinished current-node revision protection, and completed current-node emission. Targeted tests are 13/13 plus the existing privacy gate 4/4; full CWA is 2281/2281; downstream gptty is 292/292; targeted Ruff is clean.
- Installed live verification used isolated conversation `6aa54166-9cec-83ed-9baa-d4bb44535399`. A follower attached via the installed gptty environment to topic `conversation-turn-eec925e9-64ba-4d3f-9d94-571f6592f53e` while the turn was tool-active and received 29 live events, including a non-empty title-less reasoning event sourced from the repaired public `thoughts[].summary` surface. The sender completed successfully with `WK_THOUGHT_OBSERVER_OK_20260912`. Existing gptty renderer coverage already renders `message_kind="reasoning"` as the Thinking block, so no gptty source change was required.

### 2026-09-12 — active send observer parity repair

- A second user-visible failure remained after `/resume` follow was repaired: sending a new message from an already attached/resumed TUI started the elapsed timer, while ChatGPT web showed live reasoning and tool activity but the TUI remained blank. The gptty callback path was intact: `_send_chat_prompt()` already forwards every CWA `on_event` directly to `PrettyRenderer.live_event()`. The loss occurred below gptty in the WK active-send second leg.
- The WK active-send path used `_stream_resume_topic()` in token-only mode: it passed only `on_token` to `wk_transport_stream_topic()`, so raw Celsius `tool_call`, `tool_result`, thinking preamble, and `thoughts[].summary` frames never reached `CanonicalTopicStreamNormalizer`. It also retained `stop_on_done=True`, so the same path could stop at the first intermediate segment `done` even though the ChatGPT turn continued. `/resume` follow had already moved to raw events with `stop_on_done=False`, leaving the two observer paths semantically inconsistent.
- Active send now uses the same raw-observer semantics as resumed follow when the WK provider exposes the callbacks. Raw Celsius frames are normalized through `CanonicalTopicStreamNormalizer`; intermediate transport `done` markers do not terminate the subscription; the stop predicate becomes true only after the normalizer observes the canonical assistant `end_turn=true`. Token-only streaming remains the compatibility fallback for providers that do not expose the raw observer callback, and raw mode suppresses the parallel token callback to avoid duplicate assistant text events.
- Deterministic regression proves that active send forwards a public reasoning summary before final text, survives an intermediate `raw_ws_done`, and switches its stop predicate only after assistant `end_turn=true`. The lightweight transport regression independently proves `stop_on_done=False` and no duplicate token-only relay in raw mode. Targeted active-send/transport tests are 11/11; the full lightweight transport suite is 20/20; targeted Ruff is clean.
- Installed live verification continued isolated conversation `6aa54166-9cec-83ed-9baa-d4bb44535399` through the exact `GpttyClient.send_to_conversation()` path used below the TUI. The turn produced 65 observed events, including 55 intermediate events across `assistant_progress`, `reasoning`, `tool_call`, and `tool_result`. The first intermediate event arrived at 10.155 seconds, while the first assistant answer text did not arrive until 29.384 seconds and the turn completed at 34.133 seconds with `WK_ACTIVE_SEND_OBSERVER_OK_20260912`. This directly reproduces and closes the previous timer-only interval. Full CWA is 2283/2283 and downstream gptty remains 292/292. No gptty source change was required.

### 2026-09-13 — WK first-leg visibility repair

- A further live failure remained on long tool-heavy turns: a restarted installed TUI could send a new prompt, show only the elapsed `working` timer for minutes, while ChatGPT web continued to render reasoning and tools. Process inspection on the reported conversation showed the parent gptty process blocked in the browser-owned first leg with a live `WKChatGPTAuthority --observe-stream --observe-stream-until-resume-token --resume-handoff-fd ...` child for several minutes. The previously repaired raw second-leg observer had not started yet, so those fixes could not affect this interval.
- The root cause is explicit in `PassiveStreamObservationScript()`: the helper observes the protected response stream but only publishes final assistant text snapshots/deltas/revisions plus identity/handoff lifecycle signals. It does not publish tool calls, tool results, thinking preambles, or public `thoughts[].summary` intermediate nodes. `WKHelperRuntime.run_streaming()` correspondingly forwards only those text events and `write_identity_resolved`. If the resume token is delayed or absent, the TUI therefore has no intermediate event source even though canonical conversation history is already updating.
- `submit_browser_native()` now starts a canonical intermediate observer immediately after the early `write_identity_resolved` conversation identity. The observer runs concurrently with the protected WK first leg, reads canonical conversation history, emits only new post-baseline intermediate nodes through the existing revision-safe callback, and shares the same emitted-message set with `CanonicalTopicStreamNormalizer`. A lock protects canonical/raw dedupe state so a message first surfaced by canonical observation is not repeated when the raw second leg later arrives. The observer is bounded by the turn timeout and is stopped on provider completion or failure; an in-flight canonical read checks the stop flag before emitting.
- Deterministic regression holds the provider open after identity resolution and requires a canonical reasoning event to reach the consumer before provider return. The fake provider then sends the same reasoning node through a raw WebSocket event, proving canonical-first/raw-second dedupe. Targeted first-leg tests are 2/2 and targeted Ruff is clean.
- Exact installed enhanced-TUI verification used `/Users/stas/.local/bin/gptty chat`, `/resume` of completed conversation `6aa54166-9cec-83ed-9baa-d4bb44535399`, then sent a new multi-step web-tool prompt from the TUI itself. The visible `Thinking`/tool intermediate appeared at 34.842 seconds before the final answer path, producing `WK_FIRST_LEG_EXACT_GATE_OK`. This is the first acceptance gate that exercises the same interactive `prompt_toolkit -> resumed chat -> new active send -> protected WK first leg -> renderer` path as the reported failure. Full CWA is 2284/2284 and downstream gptty is 294/294.

### 2026-09-13 — attachment-only resume history repair

- The first user entry of conversation `6aa5498d-655c-83ed-ba6e-3b66385032ed` was missing from `/resume` even though the remaining transcript was present. Canonical inspection showed that the user entry is attachment-only: `content.parts=[""]`, while `metadata.attachments` contains the markdown file `Вставленная ​​уценка(3).md` (`text/markdown`, 13,881 bytes, `file_00000000b31881f49422cea91c4000f8`). Because ordinary `get_messages()` defaults to `include_empty=False`, that entire user entry was previously discarded before gptty could render it.
- The ChatGPT file resolver was verified read-only against the real attachment: `GET /backend-api/files/{file_id}/download` returns file metadata plus a download URL, and the file-content fetch succeeds when it carries the same authenticated WK transport headers. The fetched object is exactly 13,881 bytes / 13,342 UTF-8 characters and begins with the expected `@CodexTool внимательно почитай...` instructions.
- Full `/resume` snapshots (`limit=None`) now use `include_empty=True` only for the initial history read, enrich user attachment nodes, and then discard entries that are still genuinely empty. Text-like attachments are downloaded once per provider process, decoded and appended in full after a visible `[attachment: name · MIME · size]` header; non-text or temporarily unreadable attachments retain that metadata header instead of disappearing. Bounded polling/live snapshots keep the old path and do not repeatedly resolve attachment contents.
- Live verification on the reported conversation now returns 13 user messages instead of 12. The first visible user entry is the attachment entry, has 13,413 rendered characters including its header, and contains the expected instruction text. Running the exact gptty resume rendering pipeline (`_snapshot_messages -> normalize_messages -> PrettyRenderer.messages`) over that live snapshot yields 238 visible blocks, starts with the attachment-backed user entry, and contains the instruction text. Regression coverage includes authenticated attachment content fetch, provider-side attachment text caching, text attachment enrichment, and metadata-only fallback. Full CWA is 2288/2288 and downstream gptty is 295/295; targeted Ruff is clean.

### 2026-09-13 — shared canonical read throttling and cached resume

- After the observer and attachment-history repairs, repeated `/resume` attempts began failing with `WKWEBVIEW_CANONICAL_HTTP_FAILED`. Direct stage isolation showed the real first-leg failure was `GET /backend-api/conversation/{id} -> HTTP 429` in ~0.4 s. The old provider immediately treated 429 as a fallback condition and launched the WK canonical helper, which hit the same backend and collapsed the useful 429 into the opaque helper error. Three simultaneously running gptty processes plus independent active-send (2 s), resumed-follow (5 s), fallback (15 s), prewrite, and identity-recovery canonical reads could multiply request pressure. Three unrelated conversation IDs all returned the same 429, confirming account-wide throttling rather than one oversized conversation defect.
- Canonical conversation reads now share one inter-process budget through `state_dir/wk-canonical-read.lock`. The lock stores a small shared state with `next_allowed_at` and rate-limit strikes. Successful reads impose a global 5 s minimum spacing; 429 responses establish shared exponential cooldowns of 15 s, 30 s, then up to 60 s. A 429 no longer falls through to the WK helper. If the caller cannot wait out the shared cooldown within its read timeout it receives explicit `WKWEBVIEW_CANONICAL_RATE_LIMITED` with HTTP 429 semantics. Identity-recovery direct curl reads were moved behind the same coordinated gate, closing the remaining high-frequency bypass. The active-send canonical fallback watcher was reduced from 2 s to 5 s; raw Celsius remains the preferred fast path.
- Successful canonical reads now persist a best-effort, mode-0600 payload cache under `state_dir/canonical-cache/<conversation-id>.json`, with writes throttled to at most once per ~30 s per conversation. The cache is intentionally not authoritative for send, status, prewrite, or parent selection. Only read-only `conversation_follow_snapshot()` may use it when a fresh read fails specifically with 429. Cached snapshots carry `canonical_cache_stale=true` and their age; gptty `/resume` renders an explicit warning instead of presenting stale history as fresh.
- Regression coverage proves 429 -> cooldown -> successful curl retry without helper fallback; persistent 429 -> explicit `WKWEBVIEW_CANONICAL_RATE_LIMITED`; shared spacing state across separate provider instances using the same state directory; persistent payload cache round-trip; cached read-only resume on 429; refusal to use cache for non-429 failures; and the gptty stale-cache warning. Full suites are CWA 2294/2294 and gptty 296/296; targeted Ruff and diff checks are clean. Public exporter implementations independently report aggressive per-account throttling on the same conversation endpoint and use resumable raw caches plus 10-60 s backoff, matching the observed backend behavior.
